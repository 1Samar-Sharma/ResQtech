...
// WeatherGPT Master Administrator (Authenticated via chinchintu2000@gmail.com)
export const MASTER_ADMIN_EMAIL = 'chinchintu2000@gmail.com';
export const MASTER_ADMIN_NAME = 'WeatherGPT Administrator';
export const MASTER_ADMIN_PHONE = '9317230299';
export const MASTER_ADMIN_PASSWORD = 'chinchintu2000@#';

export const SECONDARY_ADMIN_EMAIL = 'sansamar2006@gmail.com';

// Public Helpline & Support
export const PUBLIC_HELPLINE_PHONE = '100 / 112 / +1 (800) 555-0199';
export const PUBLIC_HELPLINE_EMAIL = 'support@weathergpt.gov.in';
export const PUBLIC_COMMAND_NAME = 'WeatherGPT Disaster & Forecasting Center';
...
export const WEATHER_HISTORY_COLLECTION = 'weather_history';
...
class RealtimeSyncManager {
  ...
  private systemAdmins: SystemAdmin[] = [
    {
      id: 'master-admin-chintu',
      name: MASTER_ADMIN_NAME,
      email: MASTER_ADMIN_EMAIL,
      phone: MASTER_ADMIN_PHONE,
      role: 'master_admin',
      password: MASTER_ADMIN_PASSWORD,
      addedAt: 'Permanent WeatherGPT Master',
      addedBy: 'System Root',
    },
    {
      id: 'master-admin-samar',
      name: 'System Admin (Samar)',
      email: SECONDARY_ADMIN_EMAIL,
      phone: '9317230299',
      role: 'master_admin',
      password: 'chinchintu2000@#',
      addedAt: 'Co-Administrator',
      addedBy: 'System Root',
    },
  ];
...
/**
 * Save WeatherGPT query or forecast interaction to Firestore
 */
export async function saveWeatherSearchHistory(
  userId: string,
  entry: { query: string; locationName: string; persona: string }
): Promise<void> {
  if (!userId) return;
  try {
    const historyCol = collection(db, WEATHER_HISTORY_COLLECTION);
    ...
    console.warn('Firestore weather history save notice:', e);
  }
}

/**
 * Get recent WeatherGPT query history for user from Firestore
 */
export async function getWeatherSearchHistory(userId: string): Promise<any[]> {
  if (!userId) return [];
  try {
    const q = query(
      collection(db, WEATHER_HISTORY_COLLECTION),
      where('userId', '==', userId),
    ...
    console.warn('Firestore weather history fetch notice:', e);
    return [];
  }
}
