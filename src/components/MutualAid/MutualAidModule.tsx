...
export const MutualAidModule: React.FC<MutualAidModuleProps> = ({
  ...
  const handleSubmitHelpRequest = (e: React.FormEvent) => {
    ...
    onRequestHelp({
      ...
      coordinates: {
        lat: (userLocation && Number.isFinite(userLocation.lat) ? userLocation.lat : 28.6139) + (Math.random() * 0.006 - 0.003),
        lng: (userLocation && Number.isFinite(userLocation.lng) ? userLocation.lng : 77.2090) + (Math.random() * 0.006 - 0.003),
      },
      category: reqCategory,
      subCategory: reqSubCategory || (reqCategory ? reqCategory.replace(/_/g, ' ').toUpperCase() : 'GENERAL'),
      urgency: reqUrgency,
  ...
  const handleSubmitVolunteerOffer = (e: React.FormEvent) => {
    ...
    onOfferVolunteer({
      ...
      coordinates: userLocation && Number.isFinite(userLocation.lat) ? userLocation : { lat: 28.6139, lng: 77.2090 },
