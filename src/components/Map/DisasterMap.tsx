...
export const DisasterMap: React.FC<DisasterMapProps> = ({
  ...
  useEffect(() => {
    ...
    const safeLat = userLocation && Number.isFinite(userLocation.lat) ? userLocation.lat : 28.6139;
    const safeLng = userLocation && Number.isFinite(userLocation.lng) ? userLocation.lng : 77.2090;
  ...
  useEffect(() => {
    if (mapInstanceRef.current && userLocation && Number.isFinite(userLocation.lat) && Number.isFinite(userLocation.lng)) {
      mapInstanceRef.current.invalidateSize();
  ...
  useEffect(() => {
    ...
    const hasValidUserLoc = userLocation && Number.isFinite(userLocation.lat) && Number.isFinite(userLocation.lng);
    const safeUserLat = hasValidUserLoc ? userLocation.lat : 28.6139;
    const safeUserLng = hasValidUserLoc ? userLocation.lng : 77.2090;

    // 1. User Position Marker & 5km Defense Radar Circle
    if (show5kmRadar && hasValidUserLoc) {
      const isCriticalSignal = activeSignal && (activeSignal.isUrgentRedAlert || activeSignal.urgency === 'critical_urgent');
      L.circle([safeUserLat, safeUserLng], {
    ...
    <div className="relative w-full h-[calc(100vh-8.5rem)] min-h-[550px] bg-[#050810]/60 flex flex-col rounded-2xl overflow-hidden border border-white/10 shadow-2xl backdrop-blur-xl">
      ...
      {selectedEntity && (
        <div className="absolute bottom-3 right-3 z-[1000] w-full max-w-md bg-[#050810]/95 border border-white/20 text-slate-100 p-4 rounded-2xl shadow-2xl backdrop-blur-2xl animate-fadeIn max-h-[80vh] overflow-y-auto">
          ...
          {/* Distance from user */}
          {selectedEntity?.coordinates && userLocation && Number.isFinite(userLocation.lat) && Number.isFinite(selectedEntity.coordinates.lat) && (
