...
import { NWPModelComparisonView } from './NWPModelComparisonView';
import { AgroAdvisoryView } from './AgroAdvisoryView';
...
export const WeatherGPTModule: React.FC<WeatherGPTModuleProps> = ({
  ...
  // Tab State: chat | nwp | agro | imd | 7day | hourly | architecture | feasibility | climate | research
  const [activeSubTab, setActiveSubTab] = useState<
    'chat' | 'nwp' | 'agro' | 'imd' | '7day' | 'hourly' | 'architecture' | 'feasibility' | 'climate' | 'research'
  >('chat');

  // SIH 26068: NWP, Agro, and IMD Real-Time States
  ...
  const fetchNwpComparison = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.nwp) {
          setNwpData(data.nwp);
        }
      }
    } catch (err) {
      console.warn('NWP fetch warning:', err);
    } finally {
      setIsLoadingNwp(false);
    }
  };

  const fetchAgroAdvisory = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        if (data.success && data.agro) {
          setAgroData(data.agro);
        }
      }
    } catch (err) {
      console.warn('Agro advisory fetch warning:', err);
    } finally {
  ...
  const fetchImdAlerts = async () => {
    ...
    try {
      ...
      console.warn('IMD alerts fetch warning:', err);
    } finally {
      setIsLoadingImd(false);
    }
  };

  const handleAskPromptFromChild = (query: string, persona?: PersonaType) => {
    setActiveSubTab('chat');
    if (persona) {
      setSelectedPersona(persona);
    }
    setTimeout(() => {
      handleSendMessage(query, persona);
    }, 150);
  };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 pb-20 animate-fadeIn">
      ...
      {/* ========================================================================= */}
      {/* SIH 26068: IMD 4-STAGE COLOR WARNING BANNER */}
      {/* ========================================================================= */}
      <IMDAlertBanner
        alerts={imdAlerts}
        onOpenImdTab={() => setActiveSubTab('imd')}
        onAskWeatherGPT={(query) => handleAskPromptFromChild(query, 'sdma')}
      />

      {/* ========================================================================= */}
      {/* NAVIGATION SUB-TABS (COVERING ENTIRE SIH 26068 PROPOSAL & SCIENTIFIC SPEC) */}
      {/* ========================================================================= */}
      <div className="flex items-center gap-2 border-b border-white/10 pb-3 overflow-x-auto scrollbar-thin">
        ...
        <button
          onClick={() => setActiveSubTab('nwp')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap ${
            activeSubTab === 'nwp'
              ? 'bg-gradient-to-r from-sky-500 to-cyan-400 text-slate-950 shadow-lg shadow-sky-500/25'
              : 'bg-white/[0.03] text-slate-300 hover:bg-white/10 hover:text-white border border-white/5'
          }`}
        >
          <Cpu className="w-4 h-4 text-sky-400" />
          NWP Multi-Model (GFS/ECMWF/WRF)
        </button>

        <button
          onClick={() => setActiveSubTab('agro')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap ${
            activeSubTab === 'agro'
        ...
        >
          ...
          Agro-Meteorology (Kisan)
        </button>

        <button
          onClick={() => setActiveSubTab('imd')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap ${
            activeSubTab === 'imd'
              ? 'bg-gradient-to-r from-rose-500 to-amber-500 text-slate-950 shadow-lg shadow-rose-500/25'
              : 'bg-white/[0.03] text-slate-300 hover:bg-white/10 hover:text-white border border-white/5'
          }`}
        >
          <ShieldAlert className="w-4 h-4 text-rose-400" />
          IMD 4-Stage Warnings
          {imdAlerts.some((a) => a.colorCode === 'red' || a.colorCode === 'orange') && (
                ...
                {messages.map((msg) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* Message Body */}
                      {msg.sender === 'assistant' ? (
                        <WeatherResponseCard text={msg.text} />
                      ) : (
                        <div className="whitespace-pre-line text-slate-950 font-medium leading-relaxed">
                          {msg.text}
                        </div>
                      )}
      ...
      {/* ========================================================================= */}
      {/* SIH 26068 SUB-TAB: NWP MULTI-MODEL COMPARISON (GFS/ECMWF/WRF) */}
      {/* ========================================================================= */}
      {activeSubTab === 'nwp' && (
        <NWPModelComparisonView
          data={nwpData}
          isLoading={isLoadingNwp}
          onRefresh={fetchNwpComparison}
          onAskWeatherGPT={(query) => handleAskPromptFromChild(query, 'general')}
        />
      )}

      {/* ========================================================================= */}
      {/* SIH 26068 SUB-TAB: AGRO-METEOROLOGY & KISAN ADVISORY (GKMS) */}
      {/* ========================================================================= */}
      {activeSubTab === 'agro' && (
        <AgroAdvisoryView
          data={agroData}
          isLoading={isLoadingAgro}
          onRefresh={fetchAgroAdvisory}
          onAskWeatherGPT={(query) => handleAskPromptFromChild(query, 'farmer')}
        />
      )}

      {/* ========================================================================= */}
      {/* SIH 26068 SUB-TAB: OFFICIAL IMD 4-STAGE COLOR WARNING SYSTEM */}
      {/* ========================================================================= */}
      {activeSubTab === 'imd' && (
        <IMDAlertsView
          alerts={imdAlerts}
          isLoading={isLoadingImd}
          locationName={userAddress}
          onRefresh={fetchImdAlerts}
          onAskWeatherGPT={(query) => handleAskPromptFromChild(query, 'sdma')}
          onLaunchBroadcastModal={onLaunchBroadcastModal}
          onOpenAidRequestModal={onOpenAidRequestModal}
        />
      )}
    </div>
  );
