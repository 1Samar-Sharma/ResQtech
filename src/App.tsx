...
export default function App() {
  ...
  // Real-time atmospheric & forecast telemetry hook
  const {
    unit,
    toggleUnit,
    currentWeather,
    ...
    refreshImd,
    climateTrends,
  } = useWeatherData(userLocation, userAddress);
  ...
  return (
    <div className="min-h-screen bg-[#050810] text-slate-200 flex flex-col selection:bg-red-600 selection:text-white font-sans antialiased relative overflow-x-hidden pb-20 md:pb-0">
      ...
      <main className="relative z-10 flex-1 p-3 sm:p-6 max-w-7xl w-full mx-auto space-y-6">
          ...
          <div className="space-y-6">
            ...
            {mapProvider === 'google' && googleMapsApiKey && isGoogleMapsActivated ? (
              <GoogleMapsView
                ...
                onOpenBroadcastModal={() => setIsBroadcastModalOpen(true)}
                onSwitchToLeaflet={() => setMapProvider('leaflet')}
              />
            ) : (
              <DisasterMap
        ...
        {/* ========================================================================= */}
        {/* VIEW 3: FORECAST (Weather, Hourly, 7-Day & Atmospheric Telemetry) */}
        {/* ========================================================================= */}
        {activeTab === 'forecast' && (
          <ForecastScreen
            userLocation={userLocation}
            ...
            onToggleUnit={toggleUnit}
            nwpData={nwpData}
            isLoadingNwp={isLoadingNwp}
            onRefreshNwp={refreshNwp}
            agroData={agroData}
            isLoadingAgro={isLoadingAgro}
            onRefreshAgro={refreshAgro}
        ...
        {/* ========================================================================= */}
        {/* VIEW 4: ALERTS (Severe Weather, IMD 4-Stage Warnings & Early Alerts) */}
        {/* ========================================================================= */}
        ...
        {/* ========================================================================= */}
        {/* VIEW 5: CIVIC RELIEF (Mutual Aid, 5km SOS & Community Support) */}
        {/* ========================================================================= */}
        {(activeTab === 'help' || activeTab === 'mutual_aid' || activeTab === 'community') && (
          <CivicReliefScreen
            helpRequests={helpRequests}
            volunteers={volunteers}
            ...
            onOpenBroadcastModal={() => setIsBroadcastModalOpen(true)}
            onOpenSOSModal={() => setIsGlobalSOSOpen(true)}
            ...
            <div className="p-3 rounded-2xl bg-white/[0.03] border border-white/10 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  placeholder={`Lat (e.g. ${(userLocation && Number.isFinite(userLocation.lat) ? userLocation.lat : 28.6139).toFixed(4)})`}
                ...
                <input
                  ...
                  placeholder={`Lng (e.g. ${(userLocation && Number.isFinite(userLocation.lng) ? userLocation.lng : 77.2090).toFixed(4)})`}
                  className="w-full px-3 py-2 rounded-xl bg-white/[0.06] border border-white/15 text-white text-xs placeholder:text-slate-500 focus:outline-none focus:border-emerald-400 font-mono"
                />
              </div>
              <button
      ...
      <footer className="relative z-10 mt-12 border-t border-white/10 bg-[#050810]/70 backdrop-blur-xl py-6 text-center text-xs text-slate-500 hidden md:block">
